    ...
    <main className="auth-page" style={{ minHeight: "calc(100vh - 90px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 16px", background: "#f3f7f5" }}>
      <section className="auth-card" style={{ width: "100%", maxWidth: "520px", border: "1px solid #cddbd4", borderRadius: "28px", background: "#fff", padding: "32px", boxShadow: "0 24px 80px rgba(24, 59, 47, 0.10)" }}>
        ...
        <p style={{ margin: "0 0 24px", color: "#64786f", lineHeight: 1.55 }}>
          Registrieren Sie sich, um Favoriten und Suchaufträge zu speichern.
        </p>
        ...
        {error ? (
          <div style={{ marginBottom: "18px", padding: "12px 14px", borderRadius: "14px", background: "#fff4f1", border: "1px solid #f2c7bd", color: "#8a2d1a", fontWeight: 700 }}>
            Registrierung nicht möglich. Prüfen Sie Ihre Eingaben oder verwenden Sie eine andere E-Mail.
          </div>
        ) : null}

        <GoogleAuthButton href={`/api/auth/google?next=${encodeURIComponent(next)}`} label="Mit Google registrieren" />
        ...
        <form action="/api/auth/register" method="post" style={{ display: "grid", gap: "14px" }}>
          <input type="hidden" name="next" value={next} />

          <label style={{ display: "grid", gap: "8px", color: "#17372e", fontWeight: 800 }}>
            Name
            <input name="name" type="text" autoComplete="name" style={{ minHeight: "52px", borderRadius: "16px", border: "1px solid #cddbd4", padding: "0 16px", fontSize: "16px", background: "#f8fbf9" }} />
          </label>
          ...
          <label style={{ display: "grid", gap: "8px", color: "#17372e", fontWeight: 800 }}>
            Passwort
            <input name="password" type="password" required minLength={8} autoComplete="new-password" style={{ minHeight: "52px", borderRadius: "16px", border: "1px solid #cddbd4", padding: "0 16px", fontSize: "16px", background: "#f8fbf9" }} />
          </label>
        ...
        <p style={{ margin: "22px 0 0", color: "#64786f" }}>
          Bereits registriert?{" "}
          <Link href={`/login?next=${encodeURIComponent(next)}`} style={{ color: "#245e4e", fontWeight: 900, textDecoration: "none" }}>
            Anmelden
          </Link>
        </p>
      </section>
