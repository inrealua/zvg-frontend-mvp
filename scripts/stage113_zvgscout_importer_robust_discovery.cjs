...
/**
 * Stage 113 — robust ZvgScout discovery.
 *
 * Stage112 found 0 links and then timed out on page=2 because:
 * - zvgscout is likely SPA / map-driven;
 * - links may not be simple <a href="/zwangsversteigerungen/...">;
 * - networkidle can hang due to map/analytics/long polling.
 *
 * Stage113 rewrites importer:
 * - uses domcontentloaded instead of networkidle;
 * - captures JSON from network responses;
 * - extracts detail URLs from HTML, scripts, __NEXT_DATA__, JSON and all attributes;
 * - screenshots/HTML debug saved even when no links found;
 * - always writes import-summary.json, even on errors;
 * - can run discovery-only.
 */
...
if (fs.existsSync(full(pkgPath))) {
  const pkg = JSON.parse(fs.readFileSync(full(pkgPath), "utf8"));
  pkg.scripts = pkg.scripts || {};
  pkg.scripts["import:zvgscout"] = "node scripts/import_zvgscout.cjs";
  pkg.scripts["import:zvgscout:discover"] = "cross-env ZVGSCOUT_DISCOVERY_ONLY=1 ZVGSCOUT_MAX_PAGES=1 node scripts/import_zvgscout.cjs";
  ...
  fs.writeFileSync(full(pkgPath), JSON.stringify(pkg, null, 2) + "\n", "utf8");
  console.log("patched package.json");
}

console.log("Stage 113 completed.");
