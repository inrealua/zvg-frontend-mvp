...
function RadiusRange({ initialValue, locale }: { initialValue: string; locale: Locale }) {
  ...
  return (
    <div className="field span-4 radius-range-field-v50">
      ...
      <input type="hidden" id="radiusKm" name="radiusKm" value={value > 0 ? String(value) : ""} />
    </div>
  );
}
        ...
        <div className="field span-3">
          ...
          <select id="denkmalschutz" name="denkmalschutz" defaultValue={getInitialValue(searchParams, "denkmalschutz")}>
            {BOOLEAN_OPTIONS.map((option) => {
              const localized = localizeOption(option, locale);
              const label = localized.value ? localized.label : locale === "ru" ? "Не важно" : locale === "en" ? "Any" : "Egal";
              return <option key={localized.value} value={localized.value}>{label}</option>;
            })}
          </select>
        </div>
        ...
        <div className="field span-3">
          ...
          <select id="auctionAttempt" name="auctionAttempt" defaultValue={getInitialValue(searchParams, "auctionAttempt")}>
            {AUCTION_ATTEMPT_OPTIONS.map((option) => {
              const localized = localizeOption(option, locale);
              const label = localized.value ? localized.label : locale === "ru" ? "Любой термин" : locale === "en" ? "Any auction" : "Jeder Termin";
              return <option key={localized.value} value={localized.value}>{label}</option>;
            })}
          </select>
        </div>
        ...
        <div className="field span-3">
          ...
          <select id="perPage" name="perPage" defaultValue={getInitialValue(searchParams, "perPage") || "20"}>
            {PAGE_SIZE_OPTIONS.map((option) => {
              const text = option.value ? `${option.value} ${locale === "ru" ? "на странице" : locale === "en" ? "per page" : "pro Seite"}` : option.label;
